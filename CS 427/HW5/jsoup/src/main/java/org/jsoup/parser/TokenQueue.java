...
public class TokenQueue {
     ...
     * and if they do, removes the matched string from the
     * queue.
     ...
     * If the queue does not start with the supplied sequence, 
     ...
     */
    // todo: method name. not good that consumeTo cares for case, and consume to any doesn't. 
    // And the only use for this is is a case sensitive time...
    ...
    /**
     * Pulls a string off the queue (like consumeTo), and then pulls off the matched string 
     * (but does not return it).
     * <p>
     * If the queue runs out of characters before finding the seq, 
     * will return as much as it can (and queue will go isEmpty() == true).
     ...
     * Unbalanced openers and closers can be quoted (with ' or ") or escaped (with \). 
     ...
     * (tag name, but | instead of : for namespaces (or *| for wildcard namespace),
     * to not conflict with :pseudo selects).
     * 
     * @return tag name
     */
